...
namespace Shielded.Gossip.Mergeables
{
    ...
    public struct IntVersioned<T> : IMergeableEx<IntVersioned<T>>, IDeletable
    {
        [DataMember]
        ...
        public IVersion<IntVersioned<T>> GetVersionOnly() => new VersionOnly { Version = Version };

        [DataContract(Namespace = ""), Serializable]
        public struct VersionOnly : IVersion<IntVersioned<T>, VersionOnly>
        {
            [DataMember]
            public int Version { get; set; }

            public VectorRelationship CompareWithValue(IntVersioned<T> value) => Version.VectorCompare(value.Version);

            public VersionOnly MergeWith(VersionOnly other) => Version >= other.Version ? this : other;

            public VectorRelationship VectorCompare(VersionOnly other) => Version.VectorCompare(other.Version);
        }
