...
namespace Shielded.Gossip.Backend
{
    ...
    /// transactions use <see cref="RunConsistent"/> or <see cref="Prepare"/>.
    /// </summary>
    ...
    /// to the versions available on other servers. This covers all writes as well.</remarks>
    public class ConsistentGossipBackend : IGossipBackend, IDisposable
    {
        ...
        public bool ContainsKeyWithInfo(string key)
        {
            ...
            var local = _currentState.Value;
            return local.ContainsKey(key) || _wrapped.ContainsKeyWithInfo(key);
        }
        ...
        /// </summary>
        public bool TryGet<TItem>(string key, out TItem item) where TItem : IMergeableEx<TItem>
        {
        ...
        /// </summary>
        public FieldInfo<TItem> TryGetWithInfo<TItem>(string key) where TItem : IMergeableEx<TItem>
        {
            if (string.IsNullOrWhiteSpace(key))
                throw new ArgumentNullException(nameof(key));
            key = WrapPublicKey(key);
        ...
        public VectorRelationship Set<TItem>(string key, TItem item, int? expireInMs = null) where TItem : IMergeableEx<TItem>
            => Shield.InTransaction(() =>
        {
            if (string.IsNullOrWhiteSpace(key))
                throw new ArgumentNullException(nameof(key));
        ...
        private VectorRelationship SetInternal<TItem>(string key, TItem val, int? expireInMs = null) where TItem : IMergeableEx<TItem>
        {
            var local = _currentState.Value;
        ...
        {
            var local = _currentState.Value;
            if (!local.TryGetValue(key, out var oldItem) && (oldItem = (MessageItem)_wrapped.GetActiveItemWEnlist(key)) == null)
                return false;
            if (oldItem.Deleted)
                return false;
