...
namespace ConsoleTests
{
    class MainClass
    {
        ...
        static void ParallelAddWithSaving()
        {
            ...
            Shielded<int>[] shx = null;
            int[] shxClone = null;

            using (Shield.WhenCommitting<Shielded<int>>(items => {
                foreach (var ent in items)
                {
                    Thread.Sleep((ent.Value * Interlocked.Increment(ref commitEventHit) * 31) & 0x3F);
                    shxClone[Array.IndexOf(shx, ent)] = ent.Value;
                }
            }))
            {
                foreach (var i in Enumerable.Repeat(0, 10))
                {
                    shxClone = Enumerable.Repeat(0, 100).ToArray();
                    shx = shxClone.Select(n => new Shielded<int>(n)).ToArray();
                    transactionCounter = 0;
                    commitEventHit = 0;
                    ...
                    var correct = shx.Sum(s => s.Value) == taskCount &&
                                  shx.Select((s, ind) => s.Value == 0 || shxClone[ind] == s.Value).All(b => b);
